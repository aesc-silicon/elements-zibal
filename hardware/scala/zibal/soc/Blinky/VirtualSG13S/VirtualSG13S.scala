...
object VirtualSG13SBoard {
  ...
  def main(args: Array[String]) {
    ...
    val compiled = elementsConfig.genASICSimConfig.compile {
      val board = VirtualSG13SBoard(args(0))
      board
    }
    args(1) match {
      case "simulate" =>
  ...
  case class VirtualSG13SBoard(source: String) extends Component {
    val io = new Bundle {
    ...
    args(0) match {
      ...
      case _ =>
        spinalConfig.generateVerilog({
          ...
          sdc.addClock(top.io.clock.PAD, VirtualSG13SBoard.quartzFrequency)
